"""Червона команда для core.is_appledouble.

Питання власника: чи може is_appledouble ПРИХОВАТИ від доказів щось, що
не є справжнім супутником AppleDouble. Три вектори:
...
B1. Тека з іменем «._X» (не файл): is_appledouble перевіряє лише ІМ'Я,
    ...
    вмикається на зовнішніх томах — саме заради них цей продукт існує)
    перевірка стояла ПЕРЕД розрізненням файл/тека/symlink -> тека з
    таким іменем зникала з обходу цілком, разом з усім вмістом. У
    послідовному обході (walk_sequential), плані злиття (merge_plan) і
    гейті Кошика (_verify_then_trash_dir) перевірка й так стоїть лише на
    filenames з os.walk (dirnames не фільтруються) — там вектора нема;
    тести нижче доводять усі чотири точки, а не лише ту, де є фікс.

B2. Файл «._secret» БЕЗ сусіднього «secret»: is_appledouble бачить лише
    ім'я, тому гейт Кошика раніше пропускав РІВНО такий самотній файл
    без жодного доказу копії — тека з унікальними даними під іменем
    супутника їхала в Кошик мовчки. Фікс: пропуск у гейті дозволений
    лише коли в ТОМУ Ж каталозі є байтовий сусід X для «._X».
...
Фікс, що закрив ДОКАЗОВИЙ бік
(гейт Кошика), але TOCTOU-замок лишився сліпим. `core.snapshot_directory`
і `core.snapshot_directory_state` мають ТОЙ САМИЙ клас бага, що був у
`scan_one_dir` до фіксу B1: `is_appledouble(entry.name)` перевіряється ДО
розрізнення файл/тека — тека «._X» і її вміст невидимі обом Merkle-
знімкам, а самотній «._secret» (без сусіда) невидимий так само, як і в
гейті до B2. Мутація всередині такої теки МІЖ доказовим обходом гейта і
send2trash лишалась би непоміченою TOCTOU-замком «до/після»
(fsops._verify_then_trash_dir бере обидва знімки саме через
snapshot_directory_state). Фікс: пропуск дозволений лише коли запис —
звичайний файл (S_ISREG) І має байтового сусіда
(core._appledouble_sibling_exists — перенесено з fsops.py сюди, бо обидві
функції знімків живуть у core, а fsops не може бути залежністю core без
циклу; fsops._verify_then_trash_dir тепер кличе core._appledouble_
sibling_exists замість власної копії).
"""
...
def test_g1_real_companion_pair_still_excluded_from_both_snapshots(tmp_path):
    """Регресія шуму 2.18: справжня пара X+«._X» (обидва файли, сусід f.bin
    існує в тій самій теці) — «._f.bin» як і раніше НЕ входить у жоден
    знімок. macOS сама переписує супутники (напр. provenance) — TOCTOU-
    замок не повинен спрацьовувати на змінах, яких користувач не робив
    (той самий rationale, що в докстрингах цих двох функцій)."""
    root = tmp_path / "t"
