...
завжди APFS. Диск власника (BARRACUDA) — exFAT, і дефекти злиття на ньому
виявлено зі скріншотів, а не з тестів. Причина одна: жоден тест ніколи не
...
- немає жорстких посилань (`os.link` → ENOTSUP 45) — публікація переносу
  мусить іти фолбеком `_publish_no_overwrite`;
