...
def proof_is_current(expected: "FileInfo", st: os.stat_result) -> bool:
    ...
    Єдине джерело правди для порівняння метаданих. Старі сесії мають нулі
    в dev/ino — для них порівнюємо доступну пару size+mtime.
    """
    if expected is None:
        return False
    if expected.dev:
...
def verify_current_symlink(path: str) -> tuple[str, os.stat_result]:
    """Свіжий доказ для symlink-запису плану злиття.
    ...
    тексту цілі). Невідповідність гарантована і валить
    MergePreparationWorker/_transfer_files з ESTALE на будь-якому дереві
    із symlink. snapshot_directory (Merkle-доказ перед Кошиком теки) також
    окремо гілкує на stat_mod.S_ISLNK і хешує текст цілі (тег "L"), а не
    вміст. Ця функція дає той самий, повторно застосовний доказ для
    окремого symlink-запису плану переносу. Будь-яка зміна цілі між
    підготовкою і переносом дає інший digest — той самий fail-closed
            ...
            with os.scandir(dirpath) as it:
                for e in it:
                    try:
                        ...
                        # 0x00051607), ніколи не тека. Якщо is_appledouble
                        # стоятиме до розрізнення типу запису, тека з іменем
                        # «._X» зникне з паралельного обходу цілком, разом
                        # з усім вмістом.
...
def _hash_and_classify_stage(
    ...
    for size, group in by_size.items():
        if size == 0:
            ...
            # використовуємо відомий BLAKE3 порожнього вмісту. Відсутній
            # файл або файл, що став ненульовим, так само робить дерево
            # неповним.
...
def _aggregate(
    ...
    for d in all_dirs:
        # Одне зчитування замість дублю get()+[]: якщо розбити на окремі
        # guard і індексування, mypy бачить їх як незалежні виклики й не
        # визнає другий гарантовано non-None.
