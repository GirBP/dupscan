...
def perceptual_hash(path: str, hash_size: int = HASH_SIZE) -> int | None:
    ...
    Хеш інваріантний до обертань на 90/180/270 і дзеркала — обернена
    копія фото групується з оригіналом (паритет із czkawka). Механіка:
    сітка читається ОДИН раз квадратом
