...
Модуль не залежить від Qt і від app; точки перехоплення (to_trash)
приймаються параметром trash — app передає свій глобал, тому monkeypatch
app.to_trash у тестах та інтеграціях працює.
...
def to_trash(
    ...
    *,
    ...
    import send2trash
    errs: list[str] = []
    # Останній рубіж теки-еталона: навіть якщо всі
...
def _survivor_by_content(
    ...
    `_verified_survivor` їх не рятує — без цієї перевірки злиття
    переносило б такий файл у ціль, навіть коли ідентичний уже лежав там.
...
def _transfer_files(
    ...
    *,
    ...
    dict(skipped) для extra_survivors. Анотація типу — `list[tuple[str, str]]`,
    не `list[str]`: код завжди кладе сюди пари шляхів.
    """
    ...
    def report(phase: str, done: int, total: int, *, force: bool = False) -> None:
        # Гейт не обходиться зміною phase: phase містить шлях поточного
        # файла, інакше оновлення летіли б на кожному файлі й смикали
        # інтерфейс.
        now = time.monotonic()
        ...
        try:
            ...
            for number in range(1, _MAX_SUFFIX_ATTEMPTS + 1):
                ...
                if copying:
                    ...
                    # на REG чи навпаки): гейт перевіряє S_ISLNK окремо від
                    # S_ISREG, інакше перенос будь-якого symlink-запису
                    # плану провалюється.
                    still_expected_type = (
...
# узагалі. Єдиний локальний syscall, що адресує запис напряму, — rename за
...
#
# На реальному hdiutil-змонтованому exFAT-образі RENAME_EXCL підтримується
# лише частково. Контрольовано, по 3 незалежні спроби на кожен бік:
